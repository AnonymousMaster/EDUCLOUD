class InstantFeesController < ApplicationController
  ...
  def handle_category
    ...
    if params[:category_id] == "Custom"
      ...
      unless params[:category_id] ==""
        ...
        @instant_fee_particulars = @instant_category.instant_fee_particulars(:conditions => {:is_deleted=> false})
        render :update do |page|
  ...
  def create_instant_fee
    ...
    unless params[:custom_category_name].blank?
      ...
      unless params[:guest_payee].blank?
        @instant_fee.guest_payee = params[:guest_payee]
      else
    ...
    @instant_fee.pay_date = Date.today
    if @instant_fee.save
      i = 0
      @amounts = params[:amount]
      @discounts = params[:discount]
      @total_fees = params[:total]
      unless params[:particular_ids].blank?
        params[:particular_ids].each do|particular|
      ...
      unless params[:name].blank?
        @custom_particulars = params[:name]
      ...
      @transaction.save
      flash[:notice] = "Instant Fee payed successfully"
      redirect_to :action => "instant_fee_created_detail",:id => @instant_fee.id
    else
      render :action => 'new_instant_fees'
    end
  end
